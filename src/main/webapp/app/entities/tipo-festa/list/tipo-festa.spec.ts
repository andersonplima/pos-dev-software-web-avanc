...
import { sampleWithRequiredData } from '../tipo-festa.test-samples';

import { TipoFesta } from './tipo-festa';
...
describe('TipoFesta Management Component', () => {
  ...
  let comp: TipoFesta;
  let fixture: ComponentFixture<TipoFesta>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TipoFesta);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 12172 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.tipoFestas()[0]).toEqual(expect.objectContaining({ id: 12172 }));
  });

  describe('trackId', () => {
    it('should forward to tipoFestaService', () => {
      const entity = { id: 12172 };
      vitest.spyOn(service, 'getTipoFestaIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.tipoFestasParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
