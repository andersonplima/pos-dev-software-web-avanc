...
import { sampleWithRequiredData } from '../festa.test-samples';
...
describe('Festa Management Component', () => {
  ...
  let comp: Festa;
  let fixture: ComponentFixture<Festa>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Festa);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 19278 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.festas()[0]).toEqual(expect.objectContaining({ id: 19278 }));
  });

  describe('trackId', () => {
    it('should forward to festaService', () => {
      const entity = { id: 19278 };
      vitest.spyOn(service, 'getFestaIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.festasParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
